    ...
    <div className="w-[240px] bg-bkg-2 rounded-lg mt-[52px]">
      ...
      <div className="mt-8">
        {tabs.map((item) => (
          <button
            key={item}
            className="flex items-center gap-2 w-full p-4 hover:bg-bkg-1 transition-colors duration-200 ease-in-out"
          ...
          >
            ...
            <p className="text-white text-[16px] mt-1">
              {item === "accounts" ? "Manage Accounts" : chartOneUpperCase(item)}
            </p>
          </button>
        ))}
      </div>
